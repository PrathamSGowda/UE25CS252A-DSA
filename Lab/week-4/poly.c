...
void add_poly(poly_t *ptr_poly1, poly_t *ptr_poly2, poly_t *ptr_poly3)
{
	node_t *pres1 = ptr_poly1->head_;
	node_t *pres2 = ptr_poly2->head_;
	while(pres1 != NULL && pres2 != NULL)
	{
		if(pres1->term_.expo_ == pres2->term_.expo_)
		{
			insert(ptr_poly3, pres1->term_.coeff_ + pres2->term_.coeff_, pres1->term_.expo_);
			pres1 = pres1->next_;
			pres2 = pres2->next_;
		}
		else if(pres1->term_.expo_ > pres2->term_.expo_)
		{
			insert(ptr_poly3, pres1->term_.coeff_, pres1->term_.expo_);
			pres1 = pres1->next_;
		}
		else
		{
			insert(ptr_poly3, pres2->term_.coeff_, pres2->term_.expo_);
			pres2 = pres2->next_;
		}
...
void deinit_poly(poly_t* ptr_poly)
{
	node_t *pres = ptr_poly->head_;
	node_t *temp;
	while(pres)
